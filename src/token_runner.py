...
async def join_server_by_token_id(token_id: int, invite_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not invite_code:
            return {'success': False, 'msg': 'Could not parse invite URL'}
        
        invite = await client.fetch_invite(invite_code)
        guild = invite.guild
        # accept the invite
        await client.accept_invite(invite)
        return {'success': True, 'msg': f'Joined server: {guild.name if guild else invite_code}'}
    except Exception as e:
        return {'success': False, 'msg': f'Error joining server: {e}'}
...
async def send_message_by_token_id(token_id: int, user_id: int, message: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {'success': True, 'msg': f'Message sent to {user}'}
    except Exception as e:
        return {'success': False, 'msg': f'Error sending message: {e}'}
